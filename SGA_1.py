...
def gradAscent(xArr,yArr):
    ...
    for k in range(maxCycles):
        dataidex = range(m)
        for j in range(m):
            alpha = 13/(1.0+k+j)+0.001
            randidex = int(random.uniform(0,len(dataidex)))
            h = sigmoid(sum(xArr[randidex]*weights))
            error = yArr[randidex] - h
            error = float(error)
            ...
            it+=1
            iters.append(it)
            weights += alpha * xArr[randidex].transpose()*error
            del(dataidex[randidex])
